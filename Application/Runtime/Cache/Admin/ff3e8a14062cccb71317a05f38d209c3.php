...
<head>
	...
	<body>
                ...
                <div class="panel panel-default">
                     <div class="panel-heading">
					      <h3 class="panel-title">
					         ...
					         对象管理
					         <a class="right right-d2"  style="color:blue;" href="/kaohe_hzau/index.php/admin/Ceping/index"><span style="font-size:14px;color:#333;">转换到>> </span>领导个人</a>
                                 ...
                                 <thead class="top">
								   ...
								   <tbody>
								   	<?php if(is_array($listlpp)): foreach($listlpp as $key=>$listlpp): ?><tr>
								      	<td width=100px><?php echo ($listlpp["soit"]); ?></td>
										<td width=200px><?php echo ($listlpp["category"]); ?></td>
								        <td><?php echo ($listlpp["department"]); ?></td>
								           			
								        <td class="small">
								
										<a class="btn btn-xs btn-info" href="<?php echo ($listlpp["url"]); ?>" target="_blanket"><i class="icon-external-link">查看总结</i></a>
										<a class="btn btn-xs btn-info" data-toggle="modal"  data-target="#myModallpp<?php echo ($listlpp["id"]); ?>"><i class="icon-trash">编辑</i></a>
                		 
								        <a class="btn btn-xs btn-danger" href="/kaohe_hzau/index.php/admin/Ceping/delete_department?id=<?php echo ($listlpp["id"]); ?>"><i class="icon-trash">删除</i></a>
								         </td>

								
									</tr>
									<form name="formdep" method="get" action="/kaohe_hzau/index.php/admin/Ceping/update_department">
								           <!-- 模态框（Modal） -->
										<div class="modal fade" id="myModallpp<?php echo ($listlpp["id"]); ?>" tabindex="-1" role="dialog" 
										   ...
										   <div class="modal-dialog">
										      <div class="modal-content">
												 ...
												 请输入顺序（同一数字为同一类）<input name="soit" class="form-control" placeholder="请输入顺序（同一数字为同一类)" value="<?php echo ($listlpp["soit"]); ?>" type="text"/><br/>
										          请输入类别 <input name="category" class="form-control" placeholder="请输入类别" value="<?php echo ($listlpp["category"]); ?>" type="text"/><br/>
                                                  请输入测评单位 <input name="department" class="form-control" placeholder="请输入测评单位" value="<?php echo ($listlpp["department"]); ?>" type="text"/><br />
                                                  请输入测评单位的总结网址 <input name="url" class="form-control" placeholder="请输入测评单位的总结网址" value="<?php echo ($listlpp["url"]); ?>" type="text"/><br/>
										...
										<div class="modal fade" id="myModal" tabindex="-1" role="dialog" 
										   ...
										   <div class="modal-dialog">
										      <div class="modal-content">
										         ...
										         <div class="modal-body">
										         请输入顺序（同一数字为同一类）	<input name="soit" class="form-control" placeholder="请输入顺序数字（同一数字为同一类）" type="text"/><br/>
										          请输入类别 <input name="category" class="form-control" placeholder="请输入类别" type="text"/><br/>
                                                   请输入测评单位<input name="department" class="form-control" placeholder="请输入测评单位" type="text"/><br />
                                                   请输入测评单位的总结<input name="url" class="form-control" placeholder="请输入测评单位的总结网址" type="text"/>
										         </div>
										         <div class="modal-footer">
										            <button type="button" class="btn btn-default" 
										               data-dismiss="modal">关闭
										            ...
										            <input type="hidden" name="action" value="submitted"> 
										            <input type="button" class="btn btn-success" name="app" value="提交添加" onClick="submitper()"/>
										         </div>
										      </div><!-- /.modal-content -->
										</div><!-- /.modal -->
										</div>
		...
		<script type="text/javascript">
                                             ...
                                             {
                                             	//alert("hello");
                                                document.formper.action="/kaohe_hzau/index.php/admin/Ceping/set_department";
