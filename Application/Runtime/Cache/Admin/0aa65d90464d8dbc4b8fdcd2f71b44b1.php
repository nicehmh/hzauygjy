...
$(document).ready(function() 
   ...
   function f2(id){
       window.location.href="/hzauygjy/index.php/admin/people/updatepeople?id="+id;
   }
	 function delete_link(id){
	    var r=confirm("是否删除");
        if (r==true) {
               window.location.href="/hzauygjy/index.php/admin/people/delete_link?id="+id;
        }
     }
</script>
					   ...
					   <table class="table table-bordered">
                             <thead>
                               <tr >
								...
								<th style="text-align:center;">曾任职务</th>
							 ...
							 <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
								...
								<td><a href="/hzauygjy/Public/upload/<?php echo ($vo["img"]); ?>" target="_blank">查看</a></td>
                                <td>
								  <input type="button" value="修改" class="btn btn-xs" style="background-color:#B5E5EF;" onclick="f2(<?php echo ($vo["eid"]); ?>);">
								  <input type="button" value="删除" class="btn btn-xs" onclick="delete_link(<?php echo ($vo["eid"]); ?>)">
								</td>
